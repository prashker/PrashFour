...
module.exports = function(socket, connections) {
        ...
        User.findOne({username: data.username}, function(err, user) {
            ...
            if (user) {
                ...
                if (passwordHash.verify(data.password, user.password)) {
                    ...
                    socket.emit('login_success', {username: user.username, establishedConnectionExists: establishedConnectionExists});
                }
                else {
    ...
    socket.on('connect', function(data) {  
        ...
        //Sam: Removed all removeAllListeners();
        //http://stackoverflow.com/questions/13430505/do-i-have-to-remove-event-listener-when-socket-is-disconnected
        ...
        socket.on('nick', function(data) {
            ...
            connection.client.send('NICK', data.newNick);
            //Directly change .nick and opt.nick
            //https://github.com/martynsmith/node-irc/pull/139
            connection.client.nick = data.newNick;
            connection.client.opt.nick = data.newNick;
        });
