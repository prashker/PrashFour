...
module.exports = function(socket, allConnections) {
    ...
    socket.on('register', function(data) {
        ...
        User.findOne({username: data.username}, function(err, userAlreadyExists) {
            if (userAlreadyExists) {
                ...
                var user = new User({username: data.username, password: passwordHash.generate(data.password)});
                user.save(); //Save user in database
                loggedInUser = user;
                socket.emit('register_success', {message: 'Registration success', username: user.username});
            }
        });
    });
        ...
        User.findOne({username: data.username}, function(err, matchingUser) {
            ...
            if (matchingUser) {
                ...
                if (passwordHash.verify(data.password, matchingUser.password)) {
                    ...
                    matchingUser.update({lastSession: sessID}, function(err) { });
    ...
    socket.on('loginBySession', function (data) {
        User.findOne({lastSession: data.session}, function(err, matchingUser) {
            //Found a user
            if (matchingUser) {
                var establishedConnectionExists;
                loggedInUser = matchingUser;
    ...
    socket.on('connect', function(data) {  
        ...
        if (connection === undefined) {
            ...
            if (loggedInUser) {
                ...
                var dbConnection = new Connection({ user: loggedInUser.username,
        ...
        socket.on('part', function(name) {
            ...
            if (loggedInUser) {
                ...
                Connection.update({ user: loggedInUser.username }, { $pull: { channels: name.toLowerCase() } }, function(err) {});
            }
        });
        ...
        socket.on('disconnectServer', function() {
            ...
            if (loggedInUser) {
                ...
                Connection.find({user: loggedInUser.username}).remove();
                loggedInUser = null;
            }
